...
export default async function TugasSayaPage() {
  ...
  if (!siswa) {
    return (
      <div className="space-y-2">
        ...
        <p className="text-gray-500">Hanya siswa yang dapat melihat tugas.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-gray-900">Tugas Saya</h1>
      {tugas.length === 0 && <p className="text-sm text-gray-400">Belum ada tugas.</p>}
      {tugas.map((t) => {
        ...
        return (
          <div key={t.id} className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-xs text-gray-400">
                  {t.mapel ?? "-"}{fmt(t.deadline) ? ` · deadline ${fmt(t.deadline)}` : ""}
                </div>
              </div>
              {sub?.nilai != null && (
                <span className="rounded bg-green-100 px-2 py-0.5 text-xs text-green-700">Nilai: {sub.nilai}</span>
              )}
            </div>
            ...
            <form action={submitTugas} className="mt-3 space-y-2">
              ...
              <textarea name="teks" defaultValue={sub?.teks ?? ""} rows={2} placeholder="Jawaban / catatan…" className={inCls} />
              <input name="link" defaultValue={sub?.link ?? ""} placeholder="Link lampiran (opsional)" className={inCls} />
              <div className="flex items-center gap-2">
                <button className="rounded-md bg-gray-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-800">
                  {sub ? "Perbarui" : "Kumpulkan"}
                </button>
                {sub && <span className="text-xs text-gray-400">Terkumpul {fmt(sub.tanggalKumpul)}</span>}
              </div>
            </form>
          </div>
        );
