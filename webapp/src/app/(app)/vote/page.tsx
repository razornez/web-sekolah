  ...
  if (!siswa) {
    return (
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold text-gray-900">{t("voteTitle")}</h1>
        <p className="text-gray-500">{t("voteOnlyStudent")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">{t("voteTitle")}</h1>
        {sudahMemilih
          ? <p className="text-green-700">{t("voteDone")}</p>
          : <p className="text-gray-500">{t("voteInstruction")}</p>}
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {calon.length === 0 && <p className="text-sm text-gray-400">{t("voteEmpty")}</p>}
        {calon.map((c) => {
          ...
          return (
            <div key={c.id} className={`rounded-lg border p-4 ${dipilih ? "border-green-500 bg-green-50" : "border-gray-200 bg-white"}`}>
              <div className="text-xs text-gray-400">{t("voteNoUrut", { no: c.noUrut })}</div>
              ...
              {dipilih && <div className="mt-3 text-center text-sm font-medium text-green-700">{t("voteYourChoice")}</div>}
            </div>
          );
        })}
      </div>
