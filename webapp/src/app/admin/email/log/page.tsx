    ...
    <div className="space-y-6">
      ...
      <form method="get" className="flex flex-wrap gap-3">
        ...
        {(statusFilter || keyFilter) && (
          <a href="/admin/email/log" className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Reset</a>
        )}
      </form>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {logs.map((log) => (
              <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-xs text-gray-500">
                  {log.sekolah?.nama ?? <span className="italic text-gray-300">platform</span>}
                </td>
                <td className="px-4 py-3">
            ...
            {logs.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">Belum ada log email.</td>
              </tr>
            )}
          </tbody>
        </table>
