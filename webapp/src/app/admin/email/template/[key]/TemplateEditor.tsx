...
export function TemplateEditor({ template }: { template: TemplateData }) {
  ...
  // Render preview dengan contoh nilai variabel
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div className="lg:col-span-2 space-y-5">
        ...
        <form action={saveAction} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className={labelCls.replace("mb-1", "mb-0")}>Body HTML</label>
              <button
                type="button"
                onClick={() => setPreviewMode((v) => !v)}
                className="text-xs font-medium text-indigo-600 hover:text-indigo-800"
              >
                {previewMode ? "Kembali ke Editor" : "Preview dengan Data Contoh"}
              </button>
            </div>

            {previewMode ? (
              <div
                className="min-h-48 rounded-lg border border-gray-200 bg-white p-4 text-sm"
              ...
              <textarea
                ...
                rows={14}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 font-mono text-xs outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
                spellCheck={false}
              />
            )}
          </div>
          ...
          <div>
            <label className={labelCls}>Body Plain Text <span className="font-normal text-gray-400">(opsional, fallback)</span></label>
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm font-medium">
            ...
            />
            <span>Template aktif (email dari trigger ini akan dikirim)</span>
          </label>
          ...
          >
            {savePending ? "Menyimpan..." : "Simpan Template"}
          </button>
        </form>
        ...
        <form action={testAction}>
          ...
          >
            {testPending ? "Mengirim..." : "Kirim Test ke Email Saya"}
          </button>
        </form>
      </div>

      {/* Kanan: Panel Variabel */}
      <div className="space-y-4">
        <div className="rounded-xl border border-gray-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Variabel Tersedia</h3>
          <p className="text-xs text-gray-500 mb-3">Klik untuk menyalin ke clipboard.</p>
          <div className="space-y-2">
            {template.variables.map((v) => (
              ...
              >
                <code className="text-xs font-semibold text-indigo-600 group-hover:text-indigo-700">{`{{${v.name}}}`}</code>
                <p className="mt-0.5 text-xs text-gray-500">{v.description}</p>
                <p className="text-xs text-gray-400 italic">Contoh: {v.example}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {template.description && (
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Deskripsi</h3>
